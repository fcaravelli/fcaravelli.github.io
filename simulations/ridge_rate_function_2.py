...
def main(argv: list[str] | None = None) -> int:
    ...
    # Group by r = p/N (rounded) so that all (p, N) pairs sharing the
    # same ratio land on the same figure -- the theory depends only on r.
    by_r: dict[float, list[Group]] = {}
    for g in groups:
        r_key = round(g.p / g.N, args.r_decimals)
        by_r.setdefault(r_key, []).append(g)

    summary_rows: list[dict] = []

    for r_key, gs in sorted(by_r.items()):
        # Use the exact r = p/N from one of the (p,N) members for the
        # theory; if multiple unique (p,N) round to the same r_key they
        # are -- by construction -- numerically equivalent up to the
        # rounding tolerance, so any of them will do.
        unique_pn = sorted({(g.p, g.N) for g in gs})
        p_ref, N_ref = unique_pn[0]
        r = p_ref / N_ref

        pn_label = ", ".join(f"(p={p}, N={N})" for p, N in unique_pn)
        print(f"\nComputing theory for r={r:.4f}  {{{pn_label}}} ...")
        theory = compute_theory(r=r, lam=args.lam, b2=args.b2)
        print(f"  <e>_th = {theory.e_mean:.6f}, s_crit = {theory.s_crit:.4f}")

        # Filename: include all (p,N) members so two different ratios
        # never collide and the figure is self-describing.
        pn_tag = "_".join(f"p{p}N{N}" for p, N in unique_pn)
        out_png = args.out_dir / f"ridge_rate_r{r:.3f}_{pn_tag}.png"
