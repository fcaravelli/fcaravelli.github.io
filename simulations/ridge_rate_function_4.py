...
--------------------
    ...
    python ridge_rate_function_4.py --data-dir /path/to/Data --out-dir ./figures
...
for Ridge), with r = p/N and beta_i = beta_0/sqrt(p), so ||beta||^2 = beta_0^2.
...
def compute_theory(r: float, lam: float, beta0: float = 1.0,
    ...
    return TheoryCurve(r=r, lam=lam, beta0=beta0, b2=b2,
...
#          ridge_output10e9_p50_N119_beta10.txt
_PAT_FULL = re.compile(
    r"^ridge_output(?P<samples>\d+e\d+)_p(?P<p>\d+)_N(?P<N>\d+)"
...
class Run:
    ...
    N: int
    beta0: float
    rep: int           # 0 for the legacy file or beta-only file


@dataclass
class Group:
    """All replicates sharing (samples_str, p, N, beta0)."""
    samples_str: str
    ...
    def key(self) -> tuple[int, int, float]:
        return (self.p, self.N, self.beta0)
...
def discover_files(data_dir: Path,
    ...
    for entry in sorted(data_dir.iterdir()):
        ...
        run = Run(path=entry, samples_str=samples_str, samples=samples,
                  p=p, N=N, beta0=beta0, rep=rep)
        gkey = (samples_str, p, N, beta0)
...
def plot_group(r: float, lam: float,
               groups_for_r: list[Group],
               theory: TheoryCurve,
               out_path: Path,
               n_bins: int = 200,
               min_count: int = 50) -> dict:
    """Produce the two-panel figure for one value of r = p/N.

    `groups_for_r` is a list of Groups all sharing r (possibly with
    different (p, N) pairs and different sample counts).  Each Group may
    itself have several replicates that we stack.

    The empirical rate function is computed as I_emp(e) = -log(p_hat)/p,
    where p is the *group's* p (not a single global value).  The result
    is r-only by construction, so curves from different (p, N) at the
    same r should fall on the same theory curve.
    """
    ...
    for g in groups_for_r:
        s = streaming_stats([rn.path for rn in g.runs])
        stats.append((g, s))
    ...
    # Build empirical rate function for each group.  Note: -log(hist)/p
    # uses each group's own p, which is what makes the curves collapse
    # onto the r-only theory.
    curves: list[dict] = []
    for g, s in stats:
        counts, total_n = streaming_histogram([rn.path for rn in g.runs],
                                              bins=bins)
        e_emp, I_emp = _empirical_rate(counts, total_n, g.p, centers,
                                       min_count=min_count)
        curves.append({"group": g, "stats": s,
                       "e": e_emp, "I": I_emp,
                       "total_n": total_n})

    # Sort by (p, N) then total samples so the legend reads naturally.
    ...
    ax_top.plot(theory.e_grid, theory.I_grid, color="black", lw=2.2,
                label=fr"Theory  $r={theory.r:.4f}$, $\lambda={lam}$, "
                      fr"$\beta_0={theory.beta0}$")

    # Gaussian quadratic approximation around the theoretical mean.
    # Use the largest-p reference for the prefactor; this matches your
    # MATLAB recipe (-log(normpdf(...))/p) at the leading order.
    if curves:
        ref = max(curves, key=lambda c: c["total_n"])
        mu, sd, *_ = ref["stats"]
        p_ref = ref["group"].p
        I_g = (theory.e_grid - mu) ** 2 / (2.0 * p_ref * sd * sd)
        ax_top.plot(theory.e_grid, I_g, "k--", lw=1.4,
                    label=(r"Gaussian approx. "
                           rf"($p={p_ref}$, $\langle e\rangle_{{\rm MC}}$)"))

    # MC curves: distinct marker per (p, N), colour gradient by total_n
    # within each (p, N) group.
    markers = ["o", "s", "^", "D", "v", "P", "X", "*"]
    ...
    ax_top.set_title(
        fr"Ridge rate function — $r={r:.4f}$, $\lambda={lam}$, "
        fr"$\beta_0={theory.beta0}$"
        + (rf"   $(p, N) \in$ {{{pn_title}}}" if n_pn > 1
    ...
    # Restrict x-axis to where MC has meaningful content.
    if curves:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--beta0", type=float, default=1.0,
                    help="Default beta_0 for files without _beta{beta0} in the name (default: 1.0)")
    ...
    ap.add_argument("--r-decimals", type=int, default=4,
                    help="Round r=p/N to this many decimals when grouping "
                         "(default: 4, so 0.7000 == 0.7000 but 0.7001 != 0.7000)")
    ...
    for g in groups:
        ...
        print(f"  p={g.p:>4d}  N={g.N:>4d}  beta0={g.beta0:g}  "
              f"samples={g.samples_str:>5s}  reps=[{reps}]  "
              f"total={g.total_samples:.2e}")

    # Group by (rounded r, beta0) so theory is computed per unique r and beta0.
    ...
    for (r_key, beta0), gs in sorted(by_r_beta0.items()):
        ...
        print(f"\nComputing theory for r={r:.4f}, beta0={beta0:g}  {{{pn_label}}} ...")
        theory = compute_theory(r=r, lam=args.lam, beta0=beta0)
        print(f"  <e>_th = {theory.e_mean:.6f}, s_crit = {theory.s_crit:.4f}")

        pn_tag = "_".join(f"p{p}N{N}" for p, N in unique_pn)
        out_png = args.out_dir / f"ridge_rate_r{r:.3f}_{pn_tag}_beta0{beta0:g}.png"
        print(f"  Building figure -> {out_png}")
        info = plot_group(r=r, lam=args.lam,
                          groups_for_r=gs, theory=theory,
                          out_path=out_png,
                          n_bins=args.bins, min_count=args.min_count)
        for grow in info["groups"]:
            summary_rows.append({
                "p": grow["p"], "N": grow["N"], "r": r, "lambda": args.lam,
                "beta0": theory.beta0,
